...
export class InputManager{
  constructor(){
    this.keys=new Set();this.pressed=new Set();this.released=new Set();this.axes={steer:0,throttle:0,brake:0};this.gamepad=false;
    ...
    addEventListener('blur',()=>{this.keys.clear();this.axes={steer:0,throttle:0,brake:0}});
  }
  any(action,set=this.keys){return (this.bindings[action]||[]).some(k=>set.has(k))}
  down(a){return this.any(a,this.keys)} pressedAction(a){return this.any(a,this.pressed)} releasedAction(a){return this.any(a,this.released)}
  update(){
    const pads=navigator.getGamepads?.()||[];const p=[...pads].find(Boolean);this.gamepad=!!p;
    let steer=(this.down('right')?1:0)-(this.down('left')?1:0),throttle=this.down('accelerate')?1:0,brake=this.down('brake')?1:0;
    if(p){const dz=.12;const ax=Math.abs(p.axes[0]||0)>dz?p.axes[0]:0;steer=Math.abs(ax)>Math.abs(steer)?ax:steer;throttle=Math.max(throttle,p.buttons[7]?.value||0);brake=Math.max(brake,p.buttons[6]?.value||0);if(p.buttons[0]?.pressed)this.keys.add('Space');if(p.buttons[2]?.pressed)this.keys.add('KeyE');if(p.buttons[4]?.pressed)this.keys.add('KeyQ');if(p.buttons[5]?.pressed)this.keys.add('KeyN')}
...
export class AudioEngine{
  constructor(){this.ctx=null;this.master=.18;this.engineOsc=null;this.engineGain=null;this.nitroGain=null;this.lastBeat=0}
  wake(){if(!this.ctx){this.ctx=new (window.AudioContext||window.webkitAudioContext)();this._build()}if(this.ctx.state==='suspended')this.ctx.resume()}
  _build(){
    const c=this.ctx;this.engineOsc=c.createOscillator();this.engineGain=c.createGain();this.engineOsc.type='sawtooth';this.engineGain.gain.value=.0001;this.engineOsc.connect(this.engineGain).connect(c.destination);this.engineOsc.start();
