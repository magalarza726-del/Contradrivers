...
export class InputManager{
  ...
  down(a){return this.any(a,this.keys)||!!this.pad[a]} pressedAction(a){return this.any(a,this.pressed)||(!!this.pad[a]&&!this.padPrev[a])} releasedAction(a){return this.any(a,this.released)||(!this.pad[a]&&!!this.padPrev[a])}
  update(){
    ...
    if(p){const dz=.12;const ax=Math.abs(p.axes[0]||0)>dz?p.axes[0]:0;steer=Math.abs(ax)>Math.abs(steer)?ax:steer;throttle=Math.max(throttle,p.buttons[7]?.value||0);brake=Math.max(brake,p.buttons[6]?.value||0);this.pad.handbrake=!!p.buttons[0]?.pressed;this.pad.switch=!!p.buttons[2]?.pressed;this.pad.remix=!!p.buttons[3]?.pressed;this.pad.time=!!p.buttons[4]?.pressed;this.pad.nitro=!!p.buttons[5]?.pressed}
