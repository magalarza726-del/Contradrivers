...
export class TrackRuntime{
  ...
  segmentLengthAt(index,direction=1){
    const n=this.segmentLengths.length,i=wrap(Math.floor(index),n);
    return this.segmentLengths[direction>=0?i:wrap(i-1,n)]||1;
  }

  advance(index,worldDistance){
    const n=this.points.length;
    const localLen=Math.max(.05,this.segmentLengthAt(index,Math.sign(worldDistance)||1));
  ...
  nearest(position,hint=this._lastNearest,window=34){
    const n=this.points.length,origin=wrap(Math.round(hint),n);
    let best=origin,bestD=Infinity;
    for(let o=-window;o<=window;o++){
      const i=wrap(origin+o,n),p=this.points[i],dx=p.x-position.x,dz=p.z-position.z,d=dx*dx+dz*dz;
  ...
  nearestShortcut(position){
    ...
    for(const run of this.shortcutRuns){
      ...
      for(let i=0;i<run.points.length;i++){
        ...
        bestD=d;
        best={position:p,width:w,grip:+run.meta.grip||.9,distance:Math.sqrt(d),tangent:tan,heading:Math.atan2(tan.x,tan.z),pitch:Math.atan2(tan.y,planar)};
      }
    }
    return best;
  }
  ...
  isClearOfRoad(position,extra=3){
    ...
    for(const run of this.shortcutRuns){
      const clearance=(+run.meta.width||10)*0.5+extra,c2=clearance*clearance;
...
export class WorldRenderer{
  constructor(canvas,aaa){
    ...
    this.root=new THREE.Group();this.roadRoot=new THREE.Group();this.sceneryRoot=new THREE.Group();this.dynamicRoot=new THREE.Group();this.fxRoot=new THREE.Group();
    this.scene.add(this.root);this.root.add(this.roadRoot,this.sceneryRoot,this.dynamicRoot,this.fxRoot);
    ...
    this.trackRuntime=null;this.playerMesh=null;addEventListener('resize',()=>this.resize());this.resize();
  }
  ...
  clear(){[this.roadRoot,this.sceneryRoot,this.dynamicRoot,this.fxRoot].forEach(g=>this.disposeGroup(g));this.playerMesh=null;}
  ...
  makeCar(v,color=null,detail=1){
    ...
    const bodyY=Math.max(.38,sy*.5+.12);
    const body=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),bodyMat);body.name='body';body.scale.set(sx,sy,sz);body.position.y=bodyY;g.add(body);
    const nose=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),bodyMat);nose.name='nose';nose.scale.set(sx*.86,sy*.38,sz*.34);nose.position.set(0,bodyY+sy*.14,sz*.27);g.add(nose);
    const cabin=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),glass);cabin.name='cabin';cabin.scale.set(sx*.72,sy*.75,sz*.38);cabin.position.set(0,bodyY+sy*.58,-sz*.08);g.add(cabin);
    const wheelGeo=new THREE.CylinderGeometry(.31,.31,.25,detail>0?14:8);
    for(const x of [-1,1])for(const z of [-1,1]){const w=new THREE.Mesh(wheelGeo,rubber);w.rotation.z=Math.PI/2;w.position.set(x*sx*.57,.28,z*sz*.31);g.add(w);}
    const tail=new THREE.Mesh(new THREE.BoxGeometry(sx*.72,.06,.08),new THREE.MeshBasicMaterial({color:'#ff372d'}));tail.name='tail';tail.position.set(0,bodyY+.05,-sz*.505);g.add(tail);
    g.userData.vehicle=v;return g;
  }

  morphCar(mesh,a,b,t){
    const body=mesh?.getObjectByName('body'),cab=mesh?.getObjectByName('cabin'),nose=mesh?.getObjectByName('nose'),tail=mesh?.getObjectByName('tail');if(!body||!cab)return;
    const sx=lerp(a.visual_scale[0],b.visual_scale[0],t),sy=Math.max(.35,lerp(a.visual_scale[1],b.visual_scale[1],t)),sz=lerp(a.visual_scale[2],b.visual_scale[2],t),bodyY=Math.max(.38,sy*.5+.12);
    body.scale.set(sx,sy,sz);body.position.y=bodyY;
    cab.scale.set(sx*.72,sy*.75,sz*.38);cab.position.set(0,bodyY+sy*.58,-sz*.08);
    if(nose){nose.scale.set(sx*.86,sy*.38,sz*.34);nose.position.set(0,bodyY+sy*.14,sz*.27);}
    if(tail){tail.scale.x=sx*.72;tail.position.set(0,bodyY+.05,-sz*.505);}
    body.material.color.copy(new THREE.Color(a.color).lerp(new THREE.Color(b.color),t));
  }
  ...
  showMenu(v){
    ...
    this.playerMesh=this.makeCar(v);this.playerMesh.position.set(0,.065,0);this.dynamicRoot.add(this.playerMesh);this.camera.position.set(10,4.8,-14);this.camera.lookAt(0,.9,0);
  }
  ...
  showGarage(v){
    ...
    this.playerMesh=this.makeCar(v);this.playerMesh.position.y=.51;this.dynamicRoot.add(this.playerMesh);this.camera.position.set(-9.6,4.4,-10.4);this.camera.lookAt(0,1,0);
  }

  buildTrack(track){
    this.clear();const env=track.environment||{},rt=new TrackRuntime(track,this.aaa.handling?.motion_scale??.28);this.trackRuntime=rt;
    this.scene.background.set(env.sky_color||'#7894a8');this.scene.fog.color.copy(this.scene.background);this.scene.fog.near=env.fog_near||95;this.scene.fog.far=env.fog_far||440;
    ...
    for(let i=0;i<n;i++){
      ...
      pos.push(p.x+r.x*w/2,p.y+.04,p.z+r.z*w/2,p.x-r.x*w/2,p.y+.04,p.z-r.z*w/2);for(let k=0;k<2;k++)col.push(c.r,c.g,c.b);
    }
    for(let i=0;i<n;i++){const j=(i+1)%n,li=i*2,ri=li+1,lj=j*2,rj=lj+1;idx.push(li,ri,lj,ri,rj,lj);}
    const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(pos,3));geo.setAttribute('color',new THREE.Float32BufferAttribute(col,3));geo.setIndex(idx);geo.computeVertexNormals();this.roadRoot.add(new THREE.Mesh(geo,new THREE.MeshStandardMaterial({vertexColors:true,roughness:.93,metalness:.02,side:THREE.DoubleSide})));
    ...
    for(const z of track.scenery_zones||[])this.addSceneryZone(z);this.addTrackFurniture(rt);return rt;
  }

  addRoadStrip(points,width,color){
    if(points.length<2)return;const pos=[],idx=[];
    for(let i=0;i<points.length;i++){const p=points[i],prev=points[Math.max(0,i-1)],next=points[Math.min(points.length-1,i+1)],t=next.clone().sub(prev).normalize(),r=new THREE.Vector3(t.z,0,-t.x);pos.push(p.x+r.x*width/2,p.y+.035,p.z+r.z*width/2,p.x-r.x*width/2,p.y+.035,p.z-r.z*width/2);}
    for(let i=0;i<points.length-1;i++){const a=i*2,b=a+1,c=a+2,d=a+3;idx.push(a,b,c,b,d,c);}
    const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(pos,3));g.setIndex(idx);g.computeVertexNormals();this.roadRoot.add(new THREE.Mesh(g,new THREE.MeshStandardMaterial({color,roughness:.95,side:THREE.DoubleSide})));
  }
  ...
  addSceneryZone(zone){
    ...
    if(['downtown','industrial','stadium','campus','port','interchange','suburb','waterfront'].includes(type))this.addSkyline(new THREE.Vector3(x,y,z),type,32*scale,this.trackRuntime);
    else if(type==='hills'){
      for(let i=0;i<18;i++){
        const a=i/18*TAU+.37,r=24+((i*31)%57),candidate=new THREE.Vector3(x+Math.cos(a)*r,y,z+Math.sin(a)*r);if(this.trackRuntime&&!this.trackRuntime.isClearOfRoad(candidate,8))continue;
  ...
  addSkyline(center,type='city',radius=45,rt=this.trackRuntime){
    const target=Math.min(34,this.aaa.performance.max_buildings||180),geo=new THREE.BoxGeometry(1,1,1);
    ...
    const mat=new THREE.MeshStandardMaterial({color:palette[type]||'#354a59',roughness:.84,metalness:.06}),mesh=new THREE.InstancedMesh(geo,mat,target),dummy=new THREE.Object3D();let placed=0;
    for(let attempt=0;attempt<target*8&&placed<target;attempt++){
      const a=(attempt*.754877666)%1*TAU+(attempt%3)*.11,rr=radius*(.48+.68*((attempt*37)%101)/100),high=type==='downtown'||type==='interchange';
      const h=(high?8:5)+((attempt*29)%17)*(high?1.25:.72),w=2.8+((attempt*13)%7)*.55,candidate=new THREE.Vector3(center.x+Math.cos(a)*rr,center.y,center.z+Math.sin(a)*rr);
  ...
  addTrackFurniture(rt){
    ...
    for(let i=0;i<rt.points.length;i+=12){const p=rt.pose(i);for(const side of [-1,1]){const g=new THREE.Group(),post=new THREE.Mesh(new THREE.BoxGeometry(.11,3.2,.11),postMat),light=new THREE.Mesh(new THREE.SphereGeometry(.11,6,6),lampMat);post.position.y=1.6;light.position.y=3.18;g.add(post,light);g.position.copy(p.position).addScaledVector(p.right,side*(p.width*.62+1));this.sceneryRoot.add(g);}}
  }
...
export class TrafficSystem{
  ...
  reset(){
    for(const c of this.cars)this.world.dynamicRoot.remove(c.mesh);this.cars=[];const count=Math.min(this.config.pool||30,30);
    for(let i=0;i<count;i++){
      const v=this.vehicles[(i*3+1)%this.vehicles.length],mesh=this.world.makeCar(v,i%5===0?'#d7d7d7':i%5===1?'#285a86':i%5===2?'#8d3732':'#4e5c51',0);mesh.scale.multiplyScalar(.82);this.world.dynamicRoot.add(mesh);
  ...
  speedLimit(source){const cls=this.rt.classAtSource(source);if(/highway|interstate|ring/i.test(cls))return 31;if(/service|park|alley/i.test(cls))return 17;if(/hill|mountain/i.test(cls))return 21;return 24;}
  ...
  update(dt,playerIndex,levelIntensity=.5){
    ...
    for(let i=0;i<this.cars.length;i++){
      ...
      const source=this.rt.source[wrap(Math.floor(c.index),this.rt.points.length)]??0,limit=this.speedLimit(source),target=limit*c.pace*(1+levelIntensity*.08)*(c.direction<0?.9:1);
      c.speed=lerp(c.speed,target,1-Math.exp(-.65*dt));c.index=this.rt.advance(c.index,c.direction*this.rt.worldDistance(c.speed*dt));
  ...
  queryNear(position,radius=4){const out=[];for(let i=0;i<this.active;i++){const c=this.cars[i];if(c.mesh.visible&&c.mesh.position.distanceToSquared(position)<radius*radius)out.push(c);}return out;}
}
