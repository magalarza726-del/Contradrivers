...
function sanitize(value={}){return{policeEnabled:value.policeEnabled!==false,policeMax:Math.round(clamp(Number(value.policeMax)||defaults.policeMax,0,14)),uiScale:clamp(Number(value.uiScale)||1,.8,1.25)}}
function load(){let value=parse(storageGet(CURRENT_KEY));if(!value){for(const key of LEGACY_KEYS){value=parse(storageGet(key));if(value)break}}return sanitize({...defaults,...value})}

class SettingsStore extends EventTarget{
  constructor(){super();this.value=load();this.persist()}
  persist(){return storageSet(CURRENT_KEY,JSON.stringify(this.value))}
